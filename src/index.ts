...
const waltOpieValidation = new WaltOpieValidation(
  sessionManager,
  taskDistributor, 
  progressAggregator,
  safetyTiers,
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  ...
  switch (name) {
    ...
    case 'create_session': {
      const { name, profile } = args as { name?: string; profile?: string };
    ...
    case 'send_control_to_session': {
      const { sessionId, character } = args as { sessionId: string; character: string };
    ...
    case 'set_session_working_directory': {
      const { sessionId, directory } = args as { sessionId: string; directory: string };
    ...
    case 'distribute_task': {
      const { mainPrompt, subtasks } = args as { mainPrompt: string; subtasks: string[] };
      try {
    ...
    case 'create_progress_aggregation': {
      const { mainTaskId, sessionIds } = args as { mainTaskId: string; sessionIds: string[] };
    ...
    case 'update_aggregation_progress': {
      const { aggregationId, sessionId, status, output, error } = args as {
    ...
    case 'get_aggregated_progress': {
      const { aggregationId } = args as { aggregationId: string };
    ...
    case 'execute_with_retry': {
      const { sessionId, command, errorType } = args as { 
        sessionId: string; 
        command: string; 
    ...
    case 'set_session_safety_tier': {
      const { sessionId, tier } = args as { sessionId: string; tier: string };
      
      safetyTiers.setSessionSafetyTier(sessionId, tier as SafetyTier);
    ...
    case 'execute_with_safety': {
      const { sessionId, command, approvalId } = args as { 
        sessionId: string; 
        command: string; 
        approvalId?: string;
      };
    ...
    case 'approve_command': {
      const { approvalId, approvedBy } = args as { approvalId: string; approvedBy: string };
    ...
    case 'get_session_performance': {
      const { sessionId, historyLimit } = args as { sessionId: string; historyLimit?: number };
    ...
    case 'run_walt_opie_validation': {
      const { testType } = args as { testType?: 'full' | 'quick' };
