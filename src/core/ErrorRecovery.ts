...
export class ErrorRecovery {
   ...
   */
  private updateSessionHealth(sessionId: string, isHealthy: boolean): void {
   ...
   */
  private async recoverFromTimeout(context: ErrorContext): Promise<boolean> {
   ...
   * Recover from crash
   */
  private async recoverFromCrash(context: ErrorContext): Promise<boolean> {
    try {
   ...
   */
  private async recoverFromNetworkError(context: ErrorContext): Promise<boolean> {
    const delay = this.calculateRetryDelay(context.retryCount);
    await this.sleep(delay);
   ...
   */
  private async recoverFromCommandFailure(context: ErrorContext): Promise<boolean> {
    try {
   ...
   */
  private async recoverFromSessionLoss(context: ErrorContext): Promise<boolean> {
    try {
  ...
  async executeWithRetry<T>(
    sessionId: string,
    operation: () => Promise<T>,
    errorType: ErrorContext['errorType'] = 'command_failed'
  ): Promise<T> {
    ...
    while (retryCount <= this.strategy.maxRetries) {
      try {
        ...
        const context = this.createErrorContext(
          sessionId,
          error instanceof TimeoutError ? 'timeout' : errorType,
          lastError.message
        );
