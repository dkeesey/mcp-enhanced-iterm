...
export class ProcessTracker {
   ...
   */
  async monitorAllSessions(): Promise<SessionState[]> {
    const script = `
   ...
   */
  private async getProcessesForTty(tty: string): Promise<ProcessInfo[]> {
    try {
      // Get all processes for this TTY
      const { stdout } = await execAsync(
        `ps -o pid,ppid,command,%cpu,%mem -t ${tty} | tail -n +2`
      );
   ...
   */
  private parseSessionStates(output: string): SessionState[] {
