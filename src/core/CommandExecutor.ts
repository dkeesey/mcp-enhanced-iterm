...
export class CommandExecutor {
   ...
   */
  async writeToTerminal(text: string): Promise<void> {
   ...
   */
  async readTerminalOutput(lines: number = 50): Promise<string> {
    const script = `
      tell application "iTerm2"
        tell front window
   ...
   */
  async readSessionOutput(sessionId: string, lines: number = 50): Promise<string> {
    const script = `
      tell application "iTerm2"
        repeat with aWindow in windows
          repeat with aTab in tabs of aWindow
   ...
   */
  async sendControlCharacter(character: string): Promise<void> {
   ...
   */
  async sendControlToSession(sessionId: string, character: string): Promise<void> {
    const charMap: Record<string, number> = {
      'c': 3,  // Ctrl-C
      'd': 4,  // Ctrl-D
      'z': 26, // Ctrl-Z
