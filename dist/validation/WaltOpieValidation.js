import { SafetyTier } from '../core/SafetyTiers.js';
export class WaltOpieValidation {
    ...
    performanceMonitor;
    errorRecovery;
    constructor(sessionManager, taskDistributor, progressAggregator, safetyTiers, performanceMonitor, errorRecovery) {
     ...
     */
    async configureSafetyTiers(sessions) {
     ...
     */
    async distributeAnalysisTasks(sessions, prompts) {
        ...
        try {
            ...
            const aggregationId = this.progressAggregator.createAggregation(distributedTask.id, sessions);
     ...
     */
    async monitorExecution(aggregationId, timeoutMs) {
        const startTime = Date.now();
     ...
     */
    async aggregateResults(aggregationId) {
        try {
            // Generate summary
     ...
     */
    async generatePerformanceReport(sessions) {
        let totalCpu = 0;
        let totalMemory = 0;
     ...
     */
    async cleanupSessions(sessions) {
        for (const sessionId of sessions) {
            try {
    ...
    async runQuickValidationTest() {
        ...
        try {
            ...
            const result = await this.safetyTiers.executeWithSafety(session.id, 'echo "Walt Opie validation test successful"');
            if (!result.success) {
...
//# sourceMappingURL=WaltOpieValidation.js.map